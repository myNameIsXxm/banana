function PageSystem(count, divID, grountCount, callBack) {
	...
	this.init = function() {
		...
		this.pages = this.totolCount % this.pageSize == 0 ? this.pages
				: this.pages + 1;
		this.createHomePage();
		this.createPrePage();
		var n = 1;
	...
	this.toPreGroup = function() { //
		...
		if (that.currentPage == that.currentMin) { // 点了组中最小页码
			if (that.currentPage != 1) {
				...
				var pageCode = parseInt(that.currentPage)
						- (that.initMaxPage - 1); // 显示页码
				var n = 2; // 当前活动页不重创
				this.createHomePage();
				this.createPrePage();
				while (true) {
	...
	this.toEndPage = function() {
		// 去到未页
		if (that.pages == 0 || that.currentPage == that.pages)
			return;
